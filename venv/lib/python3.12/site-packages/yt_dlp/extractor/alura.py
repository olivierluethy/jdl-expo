    ...
    def _real_extract(self, url):
        ...
        if video_dict:
            ...
            video_title = clean_html(self._search_regex(
                r'<span[^>]+class=(["\'])ta__REDACTED_OPENAI_KEY__\1[^>]*>(?P<title>[^<]+)',
    ...
    def _real_extract(self, url):
        ...
        if webpage:
            for path in re.findall(r'<a\b(?=[^>]* class="[^"]*(?<=[" ])courseSectionList-section[" ])(?=[^>]* href="([^"]*))', webpage):
                ...
                for path_video in re.findall(r'<a\b(?=[^>]* class="[^"]*(?<=[" ])ta__REDACTED_OPENAI_KEY__[" ])(?=[^>]* href="([^"]*))', section_path):
                    chapter = clean_html(
                        self._search_regex(
                            r'<h3[^>]+class=(["\'])ta__REDACTED_OPENAI_KEY__\1[^>]*>(?P<chapter>[^<]+)',
                            section_path,
                            'chapter',
                            group='chapter'))
                    ...
                    chapter_number = int_or_none(
                        self._search_regex(
                            r'<span[^>]+class=(["\'])ta__REDACTED_OPENAI_KEY__[^>]*>(.*?)<strong>(?P<chapter_number>[^<]+)</strong>',
                            section_path,
                            'chapter number',
