...
const SessionPage = () => {
  ...
  const enrichQueueItem = (item) => {
    ...
    return {
      ...
      thumbnail:
        cacheEntry?.thumbnail ||
        `https://i.ytimg.com/vi/${item.fk_video_id}/mqdefault.jpg`,
    };
  };
  ...
  const loadSessionData = useCallback(async () => {
    try {
      ...
      const enrichedQueue = queueData.map((item) => {
        ...
        const cacheEntry = videoCache.find(
          (c) => c.youtubeId === item.fk_video_id,
        );
  ...
  const syncPlayback = ({ current_video_id, video_start_time, is_playing }) => {
    ...
    setCurrentSong({
      ...
      thumbnail:
        queueItem?.thumbnail ||
        `https://i.ytimg.com/vi/${current_video_id}/mqdefault.jpg`,
    });
  ...
  const proposeSong = async (video) => {
    try {
      ...
      alert(
        "Fehler beim Vorschlag: " + (err.response?.data?.error || "Unbekannt"),
      );
    }
  };
  ...
  const deleteQueueItem = async (itemId) => {
    ...
    try {
      await axios.delete(
        `https://api.tunevote.com/sessions/${sessionId}/queue/${itemId}`,
        { headers: getAuthHeaders() },
      );
        ...
        <div className="bg-white p-4 rounded-lg shadow">
          ...
          {queue.length === 0 ? (
            ...
            queue.map((item, index) => {
              const isCurrent =
                currentSong?.youtube_id === item.youtube_id && isLiveJoined;
              const isOwner =
                (userId && item.added_by == userId) ||
                (guestToken &&
                  item.guest_id &&
                  localStorage.getItem("guestToken") === guestToken);
